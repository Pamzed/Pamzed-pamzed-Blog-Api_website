...
const EditPost = ({
  ...
  const initialValues = {
    ...
    blog_feature_image: data.blog_feature_image,
    blog_short_description: data.blog_short_description,
    blog_content: data.blog_content,
    blog_categories: data.categories
      .map((c) => `${c.category_name} `)
      .join(' '),
  };

  const getSchemeForImage = (check: boolean) => {
    if (check) return Yup.mixed().required('image is required');
  ...
  const validationSchema = Yup.object({
    ...
    blog_image: getSchemeForImage(Boolean(data.blog_image)),
    blog_feature_image: getSchemeForImage(Boolean(data.blog_feature_image)),
  });
  ...
  const onSubmit = (values: any) => {
    ...
    formData.append('blog_short_description', values.blog_short_description);
    formData.append('blog_image', values.blog_image);
    formData.append('blog_feature_image', values.blog_feature_image);
    formData.append('blog_content', values.blog_content);
    formData.append('blog_id', String(blogId));
  ...
  const editPostMutation = useMutation({
    mutationFn: (data) =>
      httpService.post('/admin/edit_blog_post', data, {
        headers: {
    ...
    mutationKey: ['edit-post-mutation'],
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="grid gap-5 w-full pb-5">
      ...
      <div className="flex gap-1 mt-4">
        ...
        >
          {editPostMutation.isPending ? 'Editing' : 'Edit Content'}
        </Button>
