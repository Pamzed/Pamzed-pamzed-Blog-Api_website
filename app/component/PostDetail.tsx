...
const PostDetail = ({
  ...
  const { isLoading, isError, data, error, refetch } = useQuery({
    ...
    queryFn: () => httpService.get(`/blog/blog_detail/${id}`),
  });
  ...
  //   enabled: false,
  //   queryKey: ['related-post-query'],
  //   queryFn: () => httpService.get(`/blog/fetch_related_post/${id}`),
  // });

  useEffect(() => {
    if (isOpen) {
      refetch();
      // relatedPostQuery.refetch();
    }
  }, [isOpen]);
  ...
  // useEffect(() => {
  //   console.log('related', relatedPostQuery.data);
  // }, []);

  // useEffect(() => {
  //   if (isOpen && relatedPostQuery.data) {
  //     setPostDetail(relatedPostQuery.data.data.data);
  //     setComments(relatedPostQuery.data?.data.data.comments);
  //   }
  // }, [relatedPostQuery.data]);

  // useEffect(() => {
  //   if (relatedPostQuery.error instanceof AxiosError)
  //     toast.error(relatedPostQuery.error.response?.data.message);
  //   if (relatedPostQuery.error instanceof Error)
  //     toast.error(relatedPostQuery.error.message);
  // }, [relatedPostQuery.isError]);
              ...
              <ModalBody className="p-5 bg-white">
                <div className="grid gap-5 max-w-5xl mx-auto">
                  <div className="flex items-center text-left justify-start">
                    ...
                    {!isLoading && (
                      <h1 className="text-3xl max-w-[30ch] font-bold">
                  ...
                  <form
                    className={`mt-5 space-y-4 md:w-[55rem]`}
